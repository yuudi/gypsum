...
| 字段         | 类型              | 默认值（若有）        | 含义                                                                                                                           |
| ------------ | ----------------- | --------------------- | ------------------------------------------------------------------------------------------------------------------------------ |
| user_id      | integer           | 123456                | 测试发送消息的 QQ 号                                                                                                           |
| group_id     | integer           | 654321                | 测试发送消息的群号                                                                                                             |
| \*\*\*message_type | \*integer         | 0x0010 （普通群消息） | 测试发送消息的类型                                                                                                             |
| message      | string            |                       | （仅消息测试）测试发送的消息                                                                                                   |
| matcher_type | \*integer           | 0                     | （仅消息测试）匹配方式<br/>`0` 完全匹配<br/>`1` 关键词匹配<br/>`2` 前缀匹配<br/>`3` 后缀匹配<br/>`4` 命令匹配<br/>`5` 正则匹配 |
| pattern      | string            |                       | （仅消息测试）匹配表达式                                                                                                       |
| \*\*\*trigger_type | \*\*array\<string\> |                       | （仅事件测试）测试事件                                                                                                         |
| response     | string            |                       | 回复模板                                                                                                                       |

\* 见[消息规则](#消息规则)  
\*\* 见[触发事件](#触发事件)  
\*\*\* `message_type` 与 `trigger_type` 只能选填一个（留空可用 `null` 或不填）

| 字段    | 类型    | 含义                                                     |
| ------- | ------- | -------------------------------------------------------- |
| code    | integer | `0`表示成功，其他表示失败，失败信息在 `message` 字段获取 |
| matched | boolean | 是否成功匹配消息，事件测试中始终为 `true`                |
| reply   | string  | 发送的消息                                               |
